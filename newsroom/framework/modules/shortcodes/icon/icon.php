<?php
namespace Newsroom\Modules\Shortcodes\Icon;

use Newsroom\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Newsroom\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface
{
    ...
    public function vcMap() {
        vc_map(array(
            'name' => esc_html__('Elated Icon', 'newsroom'),
            'base' => $this->base,
            'category' => 'by ELATED',
            'icon' => 'icon-wpb-icons extended-custom-icon',
            'allowed_container_element' => 'vc_row',
            'params' => array_merge(
                \NewsroomElatedIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type' => 'dropdown',
                        ...
                        'param_name' => 'size',
                        'value' => array(
                            esc_html__('Tiny', 'newsroom') => 'eltd-icon-tiny',
                            esc_html__('Small', 'newsroom') => 'eltd-icon-small',
                            esc_html__('Medium', 'newsroom') => 'eltd-icon-medium',
                            esc_html__('Large', 'newsroom') => 'eltd-icon-large',
                            esc_html__('Very Large', 'newsroom') => 'eltd-icon-huge'
                        ),
                        'save_always' => true
                    ),
                    array(
                        'type' => 'textfield',
                        'admin_label' => true,
                        'heading' => esc_html__('Custom Size (px)', 'newsroom'),
                    ...
                    array(
                        ...
                        'value' => array(
                            ...
                            esc_html__('Circle', 'newsroom') => 'circle',
                    ...
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Border radius', 'newsroom'),
                        'param_name' => 'border_radius',
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'newsroom'),
                        'dependency' => array('element' => 'type', 'value' => array('square'))
                    ),
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Shape Size (px)', 'newsroom'),
                    ...
                    array(
                        'type' => 'colorpicker',
                        'heading' => esc_html__('Icon Color', 'newsroom'),
                    ...
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Border Width', 'newsroom'),
                        'param_name' => 'border_width',
                        'admin_label' => true,
                        'description' => esc_html__('Enter just number. Omit pixels', 'newsroom'),
                        'dependency' => array('element' => 'type', 'value' => array('circle', 'square'))
                    ),
                    array(
                        'type' => 'colorpicker',
                    ...
                    array(
                        'type' => 'colorpicker',
                        'heading' => esc_html__('Hover Icon Color', 'newsroom'),
                    ...
                    array(
                        'type' => 'dropdown',
                        'heading' => esc_html__('Icon Animation', 'newsroom'),
                    ...
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Icon Animation Delay (ms)', 'newsroom'),
                    ...
                    array(
                        'type' => 'checkbox',
                        'heading' => esc_html__('Use Link as Anchor', 'newsroom'),
                        'value' => array(esc_html__('Use this icon as Anchor?', 'newsroom') => 'yes'),
                        'param_name' => 'anchor_icon',
                        'admin_label' => true,
                        'description' => esc_html__('Check this box to use icon as anchor link (eg. #contact)', 'newsroom'),
    ...
    public function render($atts, $content = null) {
        ...
        if ($params['separator_between_icons'] == 'yes') {
            $iconHolderClasses[] = 'eltd-icon-separator';
        }
        ...
        $html = newsroom_elated_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
    ...
    private function generateIconStyles($params) {
        ...
        if (($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
            ...
            if (!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:' . newsroom_elated_filter_px($params['custom_size']) . 'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if (isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if (!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: ' . newsroom_elated_filter_px($shapeSize) . 'px';
                $iconHolderStyles[] = 'height: ' . newsroom_elated_filter_px($shapeSize) . 'px';
                $iconHolderStyles[] = 'line-height: ' . newsroom_elated_filter_px($shapeSize) . 'px';
            }
            ...
            if (!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
                ...
                $iconHolderStyles[] = 'border-width: ' . newsroom_elated_filter_px($params['border_width']) . 'px';
            ...
            if ($params['type'] == 'square') {
                if (isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: ' . newsroom_elated_filter_px($params['border_radius']) . 'px';
                }
            }
        }
