...
abstract class ListShortcode
{
    ...
    public function vcMap() {
        if (function_exists('vc_map')) {
            vc_map(array(
                ...
                "params" => newsroom_elated_get_shortcode_params($this->base)
            ));
        }
    ...
    public function renderHolders($atts, $content = null) {
        ...
        $args = newsroom_elated_get_shortcode_params_names(newsroom_elated_get_shortcode_params($this->base));
        ...
        $html .= '<div class="eltd-bnl-holder ' . $this->css_class . '-holder  ' . esc_attr($params['holder_classes']) . '" ' . newsroom_elated_get_inline_attrs($params['posts_data']) . '>';


        if (!empty($params['title']) && $params['title'] !== '') {
            $html .= '<div class="eltd-layout-title-holder">';
            $html .= newsroom_elated_execute_shortcode('eltd_section_title', array('title' => $params['title'], 'title_tag' => 'h2'));
            $html .= '</div>';
        }


        $html .= '<div class="eltd-bnl-outer">';
        ...
        if (!$this->isBlockElement()) {
            $html .= '</div>'; // close eltd-bnl-list-holder
        }

        $html .= '</div>'; // close eltd-bnl-holder-inner
    ...
    public function generatePostsQuery($params) {
        $queryResult = newsroom_elated_get_query($params);
    ...
    private function generateImageSize($params, $type) {
        ...
        if ($imageSize !== '' && $imageSize == 'landscape') {
            $thumbImageSize = 'newsroom_elated_landscape';
        } else if ($imageSize !== '' && $imageSize === 'square') {
            $thumbImageSize = 'newsroom_elated_square';
        } else if ($imageSize !== '' && $imageSize == 'portrait') {
            $thumbImageSize = 'newsroom_elated_portrait';
    ...
    private function getHolderClasses($params) {
        ...
        if (isset($params['general_style']) && $params['general_style'] !== '') {
            $holder_classes[] = 'eltd-post-layout-' . $params['general_style'];
        }

        if (isset($params['column_number']) && $params['column_number'] !== '') {
            $holder_classes[] = 'eltd-post-columns-' . $params['column_number'];
        }

        if ($this->isPaginationEnabled($params)) {
            $holder_classes[] = 'eltd-post-pag-' . $params['pagination_type'];
        }
    ...
    protected function errorMessage() {
        return '<div class="' . $this->css_class . '-message"><p>' . esc_html__('No posts were found.', 'newsroom') . '</p></div>';
    }
    ...
    private function getPaginationHtml($params, $atts) {
        ...
        switch ($params['pagination_type']) {
            case 'load-more' : {
                $html .= newsroom_elated_get_list_shortcode_module_template_part('post-show-load-more-template', 'templates', '', $atts);
            }
                break;
            case 'np-horizontal' : {
                $html .= newsroom_elated_get_list_shortcode_module_template_part('post-direction-nav-horizontal-template', 'templates', '', $atts);
                $html .= newsroom_elated_get_list_shortcode_module_template_part('post-ajax-preloader-template', 'templates', '', $atts);
            }
                break;
            default :
                break;
