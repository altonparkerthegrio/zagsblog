...
class NewsroomElatedFramework {
    ...
    private function __construct() {
        $this->eltdOptions = NewsroomElatedOptions::get_instance();
        $this->eltdMetaBoxes = NewsroomElatedMetaBoxes::get_instance();
    }
	...
	public function setSkin(NewsroomElatedSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class NewsroomElatedSkinManager
 *
...
/**
 * Class NewsroomElatedSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class NewsroomElatedSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function getMenuIcon($icon) {
		...
		return ELATED_ASSETS_ROOT.'/img/logo.ico';
	}
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo newsroom_elated_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class NewsroomElatedOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $newsroom_options)) {
            return $newsroom_options[$key];
...
/*
   Class: NewsroomElatedAdminPage
   A class that initializes Elated Admin Page
*/
class NewsroomElatedAdminPage implements iNewsroomElatedLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new NewsroomElatedFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iNewsroomElatedRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: NewsroomElatedMetaBoxes
...
/*
   Class: NewsroomElatedMetaBox
   A class that initializes Elated Meta Box
*/
class NewsroomElatedMetaBox implements iNewsroomElatedLayoutNode {
	...
	public function setTitle($label) {
		...
		return $newsroom_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'newsroom_elated_init_framework_variable' ) ) {

    function newsroom_elated_init_framework_variable() {

        global $newsroom_Framework;

        $newsroom_Framework = NewsroomElatedFramework::get_instance();
        $eltdSkinManager = new NewsroomElatedSkinManager();
        $newsroom_Framework->setSkin($eltdSkinManager->getSkin());
    }

    add_action('newsroom_elated_before_options_map', 'newsroom_elated_init_framework_variable' );
}
?>